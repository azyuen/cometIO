// cometIO audio settings + gameplay SFX v1
// Replaces the Home music toggle with a Settings popup, adds music/SFX sliders,
// encounter animation sounds, tier-up shimmer, and a sad game-over sting.
(() => {
  'use strict';
  ...
  class SFXEngine{
    constructor(){this.ctx=null;this.master=null;this.noise=null;}
      ...
      if(!this.ctx){
        ...
        this.ctx=new AC();this.master=this.ctx.createGain();this.master.gain.value=(sfxVolume/100)*.82;this.master.connect(this.ctx.destination);
    ...
    setVolume(v){sfxVolume=Math.max(0,Math.min(100,Number(v)||0));persist(SFX_KEY,sfxVolume);if(this.master&&this.ctx){const n=this.ctx.currentTime,g=this.master.gain;g.cancelScheduledValues(n);g.setValueAtTime(g.value,n);g.linearRampToValueAtTime((sfxVolume/100)*.82,n+.08);}}
    ...
    gameOver(){
      ...
      // Gentle descending minor-colour motif: clear enough to read as "run lost", not a jump scare.
      ...
      // Soft A-minor pad underneath the fall.
  ...
  function applyMusicVolume(v=musicVolume){
    ...
    try{
      ...
      if(musicVolume>0&&muted)api.toggleMute?.();
      api.setVolume?.(musicVolume/100);
    }catch(e){}
  }
  function duckMusic(mult=.12){try{window.CometSoundtrack?.setVolume?.((musicVolume/100)*mult);}catch(e){}}
  ...
  function removeLegacyMusicButton(scene){
    for(const child of [...(scene.ui?.list||[])]){
      if(!nodeHasExactLabel(child,/^MUSIC (ON|OFF)$/))continue;
  ...
  function popupButton(scene,parent,x,y,w,h,label,color,cb){
    const c=scene.add.container(x,y),g=scene.add.graphics();g.fillStyle(color,.14).fillRoundedRect(-w/2,-h/2,w,h,7);g.lineStyle(1.7,color,.90).strokeRoundedRect(-w/2,-h/2,w,h,7);const t=scene.add.text(0,0,label,{fontFamily:FONT,fontSize:'11px',fontStyle:'bold',color:'#fff'}).setOrigin(.5),hit=scene.add.rectangle(0,0,w,h,0xffffff,.001).setInteractive({useHandCursor:true});if(t.setResolution)t.setResolution(Math.min(window.devicePixelRatio||1,3));hit.on('pointerdown',cb);c.add([g,t,hit]);parent.add(c);return c;
  }
  function slider(scene,parent,label,y,getValue,setValue,color){
    const x0=72,x1=348,w=x1-x0;
    ...
    redraw(getValue());
    const hit=scene.add.rectangle((x0+x1)/2,y,w+30,42,0xffffff,.001).setInteractive({useHandCursor:true});parent.add(hit);
    ...
    popupButton(scene,parent,41,y,38,30,'−',color,()=>{const v=Math.max(0,getValue()-10);setValue(v);redraw(v);});
    popupButton(scene,parent,379,y,38,30,'+',color,()=>{const v=Math.min(100,getValue()+10);setValue(v);redraw(v);});
  }
  ...
  function showSettings(scene){
    ...
    popupText(scene,root,52,top+172,'0 = OFF   •   100 = MAX',7.2,C.muted,{bold:true});
  ...
  function addSettingsButton(scene){
    removeLegacyMusicButton(scene);
    if(!scene?.ui||typeof scene.miniButton!=='function')return;
    scene.miniButton(W-58,scene.Y(18),98,22,'SETTINGS',C.purple,()=>showSettings(scene));
  }
  ...
  wrap('showHome',function(base,args){settingsOpen=false;restoreMusic();const r=base.apply(this,args);removeLegacyMusicButton(this);addSettingsButton(this);return r;});
  ...
  // Apply saved levels once the soundtrack has had a chance to initialize.
