...
(() => {
  ...
  class UIMusicEngine{
    ...
    setVolume(v){this.volume=Math.max(0,Math.min(1,Number(v)||0));if(!this.master||!this.ctx)return;const n=this.ctx.currentTime,g=this.master.gain;g.cancelScheduledValues(n);g.setValueAtTime(g.value,n);g.linearRampToValueAtTime(this.mode?this.volume*.92:0,n+.12);}
    start(mode){if(mode!=='home'&&mode!=='lab')return;if(!this.unlocked){this.pending=mode;this.mode=mode;return;}if(!this.ensure())return;if(this.mode===mode&&this.timer)return;this.stop(.28,false);this.mode=mode;this.bar=0;this.next=this.ctx.currentTime+.06;const n=this.ctx.currentTime,g=this.master.gain;g.cancelScheduledValues(n);g.setValueAtTime(Math.max(.0001,g.value),n);g.linearRampToValueAtTime(Math.max(.0001,this.volume*.92),n+.55);this.tick();this.timer=setInterval(()=>this.tick(),100);if(mode==='lab')try{window.CometSoundtrack?.setVolume?.(0);}catch(e){}
    }
    stop(fade=.45,restore=true){this.pending=null;if(this.timer){clearInterval(this.timer);this.timer=null;}if(this.master&&this.ctx){const n=this.ctx.currentTime,g=this.master.gain;g.cancelScheduledValues(n);g.setValueAtTime(Math.max(.0001,g.value),n);g.linearRampToValueAtTime(.0001,n+fade);}this.mode=null;if(restore){const v=getSaved();try{window.CometSoundtrack?.setVolume?.(v*2);}catch(e){}}}
